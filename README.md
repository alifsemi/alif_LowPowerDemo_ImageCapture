...
This template application is built on top of the VSCode Getting Started Template (alif_vscode-template), but it is delivered for Alif Ensemble E7 and E8 Devkits only (Devkit-e7, Devkit-e8). The new **lp_cam** project is added to demonstrate a GPIO triggered image capture with dynamic frequency scaling to reduce system-level power when idling in between frame captures.
